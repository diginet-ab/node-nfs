...
function LookupReply(opts) {
    ...
    this._nfs_lookup_reply = true; // MDB
}
util.inherits(LookupReply, NfsReply);
LookupReply.prototype._allowed_error_codes = [
    nfs_err.NFS3ERR_IO,
...
LookupReply.prototype.setAttributes = function setAttributes(stats) {
    assert.ok(stats instanceof BrowserFs.BFS.fs.FS.Stats, 'fs.Stats');

    this.obj_attributes = fattr3.create(stats);

    return (this.obj_attributes);
};
LookupReply.prototype.setObjAttributes = LookupReply.prototype.setAttributes;


LookupReply.prototype.setDirAttributes = function setDirAttributes(stats) {
    assert.ok(stats instanceof BrowserFs.BFS.fs.FS.Stats, 'fs.Stats');
...
LookupReply.prototype._transform = function _transform(chunk, enc, cb) {
    if (this.incoming) {
        var xdr = new XDR(chunk);

        this.status = xdr.readInt();
        if (this.status === 0) {
            this.object = xdr.readString();
            if (xdr.readBool())
...
LookupReply.prototype.writeHead = function writeHead() {
    var len = 8;
    ...
    if (this.status === 0) {
        len += XDR.byteLength(this.object) + 4;
    ...
    function write_attrs(attrs) {
        if (attrs) {
            xdr.writeBool(true);
            fattr3.serialize(xdr, attrs);
        } else {
            xdr.writeBool(false);
        }
    }
    ...
    if (this.status === 0) {
        xdr.writeString(this.object);
        write_attrs(this.obj_attributes);
        write_attrs(this.dir_attributes);
    } else {
        write_attrs(this.dir_attributes);
    }
...
LookupReply.prototype.toString = function toString() {
    var fmt = '[object LookupReply <xid=%d, status=%d, object=%s, ' +
        'obj_attributes=%j, dir_attributes=%j>]';
