...
function AccessReply(opts) {
    ...
    this._nfs_access_reply = true; // MDB
}
util.inherits(AccessReply, NfsReply);
AccessReply.prototype._allowed_error_codes = [
    nfs_err.NFS3ERR_IO,
...
AccessReply.prototype.setAttributes = function setAttributes(stats) {
...
AccessReply.prototype._transform = function _transform(chunk, enc, cb) {
    if (this.incoming) {
...
AccessReply.prototype.writeHead = function writeHead() {
    var len = 8;
...
AccessReply.prototype.toString = function toString() {
    var fmt = '[object AccessReply <xid=%d, status=%d, attributes=%j, ' +
        'access=%d>]';
