...
function CommitCall(opts) {
    ...
    this._nfs_commit_call = true; // MDB
}
util.inherits(CommitCall, NfsCall);
Object.defineProperty(CommitCall.prototype, 'object', {
    get: function object() {
        return (this.file);
    }
});
...
CommitCall.prototype._transform = function _transform(chunk, enc, cb) {
    if (this.incoming) {
...
CommitCall.prototype.writeHead = function writeHead() {
    var len = 12 + XDR.byteLength(this.file);
...
CommitCall.prototype.toString = function toString() {
    var fmt = '[object CommitCall <xid=%d, file=%s, offset=%d, count=%d>]';
