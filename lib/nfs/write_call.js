...
function WriteCall(opts) {
    ...
    assert.optionalNumber(opts.stable, 'options.stable');
    assert.optionalObject(opts.data, 'options.data');
    ...
    this.stable = opts.stable || 0;
    this.data = opts.data || null;

    this._nfs_write_call = true; // MDB
}
util.inherits(WriteCall, NfsCall);
Object.defineProperty(WriteCall.prototype, 'object', {
    get: function object() {
        return (this.file);
    }
});
...
WriteCall.prototype._transform = function _transform(chunk, enc, cb) {
    if (this.incoming) {
...
WriteCall.prototype.writeHead = function writeHead() {
    var len = 16 + XDR.byteLength(this.file) + XDR.byteLength(this.data);
...
WriteCall.prototype.toString = function toString() {
    var fmt = '[object WriteCall <xid=%d, file=%s, offset=%d, count=%d, ' +
        'stable=%d, nbytes=%d>]';
    return (util.format(fmt, this.xid, this.file, this.offset, this.count,
       this.stable, this.data.length));
};
